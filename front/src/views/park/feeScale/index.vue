        ...
        <Row v-show="openSearch" @keydown.enter.native="handleSearch">          <Form ref="searchForm" :model="searchForm" inline :label-width="0">
          <Form-item
            ...
            >
              ...
              <Form-item label="" prop="startPrice">
                <Input type="text" v-model="searchForm.startPrice" placeholder="起步费用" clearable style="width: 150px"/>
              </Form-item>
              <Form-item label="" prop="beyondPrice">
                <Input type="text" v-model="searchForm.beyondPrice" placeholder="超时费用" clearable style="width: 150px"/>
              </Form-item>
...
<script>
import { getFeeScaleList, deleteFeeScale } from "./api.js";
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getFeeScaleList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
            deleteFeeScale({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          ids = ids.substring(0, ids.length - 1);
            deleteFeeScale({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
