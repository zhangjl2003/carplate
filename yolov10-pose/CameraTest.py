...
def insert_parking_info(car_number, entry_time, fee_id):
    ...
    try:
        # 发送POST请求
        response = requests.post(url, json=new_parking_info)

        # 检查响应状态码
        if response.status_code == 200:
            # 尝试解析响应内容，假设返回的是JSON格式
            result = response.json()
            print("操作成功，响应数据:", json.dumps(result, indent=4))
            return result
        else:
            # 打印错误信息并返回None表示操作失败
        ...
        print(f"请求过程中发生错误: {e}")
        return None
# insert_parking_info("苏A12345", "2023-05-01 12:00:00", 1)
    ...
    if success:
        ...
        if len(location_list) >= 1:
            ...
            if frame_counter == 30:
                ...
                # 写入数据库逻辑
                insert_parking_info(most_common_plate)
